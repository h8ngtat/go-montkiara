...
@interface HTAaMicroservicesEshopClient()
...
@implementation HTAaMicroservicesEshopClient

static NSString *const AWSInfoClientKey = @"HTAaMicroservicesEshopClient";
...
+ (instancetype)defaultClient {
    ...
    static HTAaMicroservicesEshopClient *_defaultClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _defaultClient = [[HTAaMicroservicesEshopClient alloc] initWithConfiguration:serviceConfiguration];
    });
...
+ (void)registerClientWithConfiguration:(AWSServiceConfiguration *)configuration forKey:(NSString *)key {
    ...
    [_serviceClients setObject:[[HTAaMicroservicesEshopClient alloc] initWithConfiguration:configuration]
                        forKey:key];
}

+ (instancetype)clientForKey:(NSString *)key {
    @synchronized(self) {
        HTAaMicroservicesEshopClient *serviceClient = [_serviceClients objectForKey:key];
        if (serviceClient) {
        ...
        if (serviceInfo) {
            AWSServiceConfiguration *serviceConfiguration = [[AWSServiceConfiguration alloc] initWithRegion:serviceInfo.region
                                                                                        credentialsProvider:serviceInfo.cognitoCredentialsProvider];
            [HTAaMicroservicesEshopClient registerClientWithConfiguration:serviceConfiguration
                                                    forKey:key];
        }
...
- (instancetype)initWithConfiguration:(AWSServiceConfiguration *)configuration {
    if (self = [super init]) {
        ...
        NSString *URLString = @"https://1c1wz4ejwe.execute-api.ap-southeast-1.amazonaws.com/Stage";
